...
x[num_times / 2] = gsl_vector_get(v,2);
double alpha = gsl_vector_get(v,1)* scale_factor;
double alpha2 = gsl_vector_get(v,1)* scale_factor;
printf("x[0] =  %.15f\n", x[0]);
for (j=1; j<num_times / 2; j++)
{
x[j] = x[j-1] * exp(-alpha * ( time[j] - time[j-1] ));
printf("x_j = %f\n", x[j]);
if ( x[j] < 100 )
{
return 100000000000;
...
for (j=num_times / 2 + 1; j < num_times; j++)
{
x[j] = x[j-1] * exp(-alpha2 * ( time[j] - time[j-1] ));
printf("x_j = %f\n", x[j]);
if ( x[j] < 100 )
{
return 100000000000;
...
double delta = 1;
double pop_delta = 100;
double alpha = gsl_vector_get(v,1) * scale_factor;
double alpha2 = gsl_vector_get(v,1) * scale_factor;
x[0] = gsl_vector_get(v,0);
y[0] = x[0] + pop_delta;
x[num_times / 2] = gsl_vector_get(v,2);
y[num_times / 2] = x[num_times / 2] + pop_delta;
// change initial pop

for (j=1; j<num_times / 2; j++)
{
	x[j] = x[j-1] * exp(-alpha * ( time[j] - time[j-1] ));
	y[j] = y[j-1] * exp(-alpha * ( time[j] - time[j-1] ));
}

for (j=num_times / 2 + 1; j < num_times; j++)
{
	x[j] = x[j-1] * exp(-alpha2 * ( time[j] - time[j-1] ));
	y[j] = y[j-1] * exp(-alpha2 * ( time[j] - time[j-1] ));
}

gsl_vector_set(df, 0, (1.0 / pop_delta) * ( Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p) ) );
//printf("diff = %.15f\n", Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p));
...
gsl_vector_set(df,1, (1.0 / pop_delta) * ( Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p) ) );
...
// change first growth rate

double alpha_delta = 1e-15;
for (j=1; j<num_times / 2; j++)
{
	y[j] = y[j-1] * exp(-(alpha+alpha_delta) * ( time[j] - time[j-1] ));
}


gsl_vector_set(df, 2, (1.0 / alpha_delta) * ( Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p) ) );
//printf("diff2 = %.15f\n", Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p));
...
// change second growth rate

for (j=num_times / 2 + 1; j < num_times; j++)
{
	y[j] = y[j-1] * exp(-(alpha2+alpha_delta) * ( time[j] - time[j-1] ));
}


gsl_vector_set(df, 3, (1.0 / alpha_delta) * ( Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p) ) );
//printf("diff2 = %.15f\n", Joint_like(num_times, time, y, p) - Joint_like(num_times, time, x, p));
...
int num_vars;
num_vars = num_pop_sizes + num_growth_rates;
...
// likelihood setting
  gsl_multimin_fdfminimizer_set (s, &my_func, x, 10000, .1);
...
printf ("Iteration terminated at:\nSTART HERE\n");
	...
	//printf ("%5zu ", iter);  // using %zu instead of %d b/c iter is type size_t not type int
			int k=0;
				for (k = 0; k < num_pop_sizes; k++)  
 			{
 			printf ("%10.15f\n", gsl_vector_get (s->x, k)); // problem XYRQ
 						// using k here renders size_t i above unused
 			}
			for (k = 0; k < num_growth_rates; k++)  
 			{
 			printf ("%10.15f\n", .00001* gsl_vector_get (s->x, num_pop_sizes +k)); // problem XYRQ
 						// using k here renders size_t i above unused
 			}
	printf ("f() = %7.3f \n", s->f);
//  q
