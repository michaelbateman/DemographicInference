...
double time[pop_dim];
double q[M][M][pop_dim];
double r[M][pop_dim];
double product[M][M][pop_dim];
double product2[M][M][pop_dim];
double product3[M][pop_dim];
double product4[M][M][pop_dim];

double C1[M][M][pop_dim];
double Zfirst1[M][M][pop_dim];
double Zsecond1[M][M][pop_dim];

double C2;
double Zfirst2[pop_dim];
double Zsecond2[pop_dim];

double H1[M][pop_dim];
double Yfirst1[M][pop_dim];
double Ysecond1[M][pop_dim];

double H2;
double Yfirst2[pop_dim];
double Ysecond2[pop_dim];

double I1[M][pop_dim];
double Wfirst1[M][pop_dim];
double Wsecond1[M][pop_dim];

double I2;
double Wfirst2[pop_dim];
double Wsecond2[pop_dim];

double G_exp[M][pop_dim];
double G_exp_2[M][pop_dim];
...
double exp_part_L[M];
double exp_part_other[M][M][pop_dim];


static int pop_dim =6;
*/
...
//printf("time(%d) = %f\n", j, time[0]);
    ...
    {
time[j] = 1.0 *  j / num_times * 2.0 *  (20000 / pairs); 
...
long double G(int num_haps, int num_times,int i, double L, double T[pop_dim], double lambda[num_times], double var1 )
{
...
//G_vector[j] = G(2, num_times, 1, j * delta + 1,  T, lambda, mu);
...
{ 
	...
	for(m =1; m< j; m++)
	{
		total = total + delta * F_vector[m] * F_vector[j  - m];
		//total = total + delta * F_vector[m] * G_vector[j  - m];
		//printf("FconvF (%d) = %f\n", 
	}
	F_conv_F_vector[j] = total;
	//F_conv_G_vector[j] = total;
}
...
//printf("help %f\n", 1.0);
		...
		//double temp4 = two_fire_prob(num_haps, num_times, bin_center[l], T, pop_size, lambda);
...
for (j=0; j<num_times; j++)
{
if(j<=50){gsl_vector_set (x, j, 5000  );}
