...
double Z(int num_times, int m, double B)
{
//printf("time = %f\n", time[m+1]);
//printf(" B = %.20f\t\tZ = %.20f\n",B,  (exp(-time[m] * B) - exp(-time[m+1] * B) ) / B  );

if(m == num_times - 1)
{
return( exp(-time[m] * B) / B );
}
else
{
return( ( exp(-time[m] * B) - exp(-time[m+1] * B) ) / B );
}
}
...
//printf("help %f\n", 1.0);
		...
		printf("%.20f\t REPRINT \t\t mu = %.14Lf\n", temp6, l, bin_center[l], mu);
			...
			else{joint = joint + log( temp4 ) * bin_counter[l];}// * l;} // extra factor of l in place to account for the collection of all matches rather than only s
	//	printf("temp4 = %.20f\n", temp4);
...
for (j=0; j<num_times; j++)
{
if(j<=50){gsl_vector_set (x, j, 10000  );}
...
// likelihood setting
  gsl_multimin_fdfminimizer_set (s, &my_func, x, 1000000, .1);
