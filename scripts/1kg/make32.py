...
for pop in pop_list:
	...
	call_string = 'time ../bcftools/bcftools view ' + pop_string + ' -o combo32.vcf'+ ' -s ' + total_string
	print call_string
	subprocess.call(call_string, shell=True)
	#subprocess.call('../bcftools/bcftools view', s1, '-h')
	max_file = str(num_samples)+'.'+str(k) +'.'+ pop + '.max'
	print max_file
	call_string = '../manyhaps/pbwt/pbwt -readVcfGT combo32.vcf -write combo32.pbwt -writeSites combo32.sites -maxWithin > ' + max_file
	print call_string
	subprocess.call(call_string, shell=True)
	call_string = 'rm combo32.pbwt'
	subprocess.call(call_string, shell=True)
	call_string = 'rm combo32.sites'
	subprocess.call(call_string, shell=True)
	call_string = 'rm combo32.vcf'
	subprocess.call(call_string, shell=True)
