pop_list = [ 'AK','AV','AR', 'AS', 'ABM','ABS','AC', 'AJ','AN',   ] 
...
for i in range(0,1):   # Extra loop here in case we want a loop over regions of the genome
	...
	if (num_samples == 1):
		...
		max_file = pop_file + '.' + str(num_samples) +'.'+ str(i) + '.max'
	    	...
	    	call_string = '../../pbwtCurrent/pbwt/pbwt -readAll ' + pop_file  + ' -selectSamples ' + sample_file + ' -maxWithin > ' + max_file 
	    	print call_string
	    	subprocess.call(call_string, shell=True)	
			
			
		#call_string = '../../pbwtCurrent/pbwt/pbwt -readAll ' + pop_file  + ' -selectSamples ' + sample_file + ' -subsites .999999999999 .4' + ' -maxWithin > ' + max_file 
	    	#print call_string
	    	#subprocess.call(call_string, shell=True)	
