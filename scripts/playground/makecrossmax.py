...
for i in range(0,int(num_regions)):
    ...
    for pop in pop_list:
	    ...
	    print max_file
	    call_string = '../pbwtCross/pbwt/pbwt -readVcfGT ' + f + '.vcf -write ' + f + '.pbwt -writeSites ' + f + '.sites -maxWithin > ' + max_file
	    print call_string
	    subprocess.call(call_string, shell=True)
	    ...
	    call_string = 'mv ' + max_file + ' ./maxfiles/cross/'+str(num_samples)
	    subprocess.call(call_string, shell=True)
	    k_pop += 1
	    k+=1
